...
The service SHALL cache product detail responses in Redis on port 6739 with key pattern `product:detail:{id}` and `product:detail:slug:{slug}` with an expiration TTL.
...
- **THEN** the system deletes `product:detail:{id}` and associated slug keys from Redis before completing the HTTP response.
...
- **WHEN** Redis is down or unreachable on port 6739

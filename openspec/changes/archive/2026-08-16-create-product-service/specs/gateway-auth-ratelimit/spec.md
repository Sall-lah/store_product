...
The service SHALL enforce that write endpoints (`POST`, `PUT`, `DELETE` on `/api/v1/products`) verify the presence of `X-User-Role: admin`.

#### Scenario: Authorized admin request
- **WHEN** a client sends a write request with header `X-User-Role: admin` and `X-User-Id: <user_id>`
...
- **WHEN** a client sends a write request with missing `X-User-Role` or `X-User-Role` value other than "admin"
